...
(test cube-conundrum
  (let ((color-config (list 12 13 14))
	...
	(game-strings-from-input-file (uiop:read-file-lines
    ...
    (is (= 2512 (cube-conundrum color-config game-strings-from-input-file)))

    (is-true (zerop (sum-powers '())))
    (is-true (zerop (sum-powers '(""))))
    (is-true (= 2286 (sum-powers example-game-strings)))
    (is-true (= 67335 (sum-powers game-strings-from-input-file)))))
